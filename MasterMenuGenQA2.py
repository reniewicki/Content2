...
import codecs, os.path, sys, re, shutil, time, getpass, os, distutils, random, xlrd, xlwt, glob, unicodedata, difflib, xlutils
...
def EditCTR(sheet, row, col, value):
	if (os.path.exists(ctr_filename[l][0])):
		ctr_xl = open_workbook(ctr_filename[l][0], formatting_info=True)
	else:
		print "Excel sheet does not exist: CTR_thatana_2014-08_eng.xls"
		exit()
	write_ctr_xl = copy(ctr_xl)
	write_ctr_xl.get_sheet(sheet).write(row, col, value)
	os.remove(ctr_filename[l][0])
	write_ctr_xl.save(ctr_filename[l][0])
		...
		if (len(key) > 1):
			...
			if (icon.lower() == "random-poster"):
				for i in range(0, len(menu_metadata[l][c])):
					if (menu_metadata[l][c][i][4].rstrip('"').lstrip('"').strip().replace("#","@") == key):
						r = i
						break
				print menu_metadata[l][c][r][5]
...
def FindBestFileNameMatch(filebase, langs, file_list):
	...
	if (new_holdover == "H"):
		...
		threshold = .9
	if (menu_metadata[l][c][i][16] != "" or menu_metadata[l][c][i][17] != ""):
	...
	for k in range(len(file_list)):
		full_match = SequenceMatcher(None, filebase.replace("The", "") + video_date + langs, file_list[k][:-9].replace("The", ""))
		if (full_match.ratio() > .95):
			return file_list[k][:-5]
		file_list_splits = file_list[k][:-9].split("_")
		if (has_subs):
			file_langs_split = SplitUpLangs(file_list_splits[-2])
			file_subs_split = SplitUpLangs(file_list_splits[-1])
			if (not HasLangs(item_subs_split, file_subs_split)):
				continue
			if (not HasLangs(item_langs_split, file_langs_split)):
				continue
		else:
			file_langs_split = SplitUpLangs(file_list_splits[-1])
			if (not HasLangs(item_langs_split, file_langs_split)):
				continue
		normalized_filebase = TitleToFilename(filebase.replace("The", "").lower() + video_date)
		normalized_file_list = TitleToFilename(file_list_splits[0].replace("The", "").lower())
		title_match = SequenceMatcher(None, normalized_filebase, normalized_file_list)
		
		if (title_match.ratio() >= threshold):
			canidates.append(file_list[k])
			canidate_ratio.append(title_match.ratio())
		if ((title_match.ratio() + full_match.ratio()) > highest_ratio):
			highest_match = k
			highest_ratio = title_match.ratio() + full_match.ratio()
			highest_title_ratio = title_match.ratio()
	if (len(canidates) > 1):
		print "\nSelect video file. Expecting: " 
		print filebase + video_date + langs + ".mkv"
		print "\nEnter selection #:"
		for can in range(len(canidates)):
			print "\n#" + str(can + 1) + " - " + canidates[can][:-5] + " - " + str(canidate_ratio[can])
		print "\n(n/N) for None"
		selection = input("Enter selection: ")
		if (isinstance(selection, int) and selection <= len(canidates)):
			return file_list[(selection - 1)][:-5]
		return "MISSINGVIDEO"
...
def GetImage(local_title, local_episode, directory):
	...
	if(os.path.exists(__GenerationDir + __GraphicsLibDir + directory + "\\" + ImageFullfilebase + ".png")):
		...
		__MissingFiles.append(ImageFullfilebase + ".png" + directory)
...
def GetAdList(ad_one, ad_two):
	...
	if (ad_one != ""):
		if (avail_ad_list != ""):
			ads = " /content/" + ad_dir + "/" + FindHighestFileNameMatch(ad_one, avail_ad_list)
		else:
			ads = " /content/" + ad_dir + "/MISSINGAD"
	if (ad_two != ""):
		if (isinstance( ad_two, float )):
			ad_two = str(int(ad_two))
			if (ad_two == "6"):
				ad_two = "06"
			ads += " /content/" + ad_dir + "/AFL-Disclaimers8sec16x9-" + ad_two + "andUp_SIL.mkv"
		else:
			ads += " /content/" + ad_dir + "/MISSINGAD"
	return ads
...
if __name__ == "__main__":
	...
	if (len(sys.argv) != 5):
		print "Usage: MasterMenuGen.py <outputdir ex: ./> <QA1 or QA2> <year ex: 2014> <month ex: 09>"
	...
	QA = sys.argv[2]
	ctr_date = sys.argv[3] + "-" + sys.argv[4]
	...
	ctr_filename = glob.glob(__CTR_dir + "/CTR_" + airline_name + "_" + ctr_date + "_eng.xls")
	if (len(ctr_filename) < 1):
			print "CTR_" + airline_name + "_" + ctr_date + "_eng.xls is missing"
			exit()
	...
	if ("video" in start_up_ad):
		start_up_ad += "  /content/" + ad_dir + "/" + ctr_excel_list[overview_page][2][2].replace(".mkv", "").strip() + ".mkv"
	elif ("billboard" in start_up_ad):
		start_up_ad = "!" + start_up_ad + "  /content/menu/images/" + ctr_excel_list[overview_page][2][2].replace(".png", "").strip() + ".png 10"
	after_lang_ad = ctr_excel_list[overview_page][3][1].strip()
	if ("video" in after_lang_ad):
		after_lang_ad += "  /content/ads/" +  ctr_excel_list[overview_page][3][2].replace(".mkv", "").strip() + ".mkv"
	elif ("billboard" in after_lang_ad):
		after_lang_ad = "!" + after_lang_ad + "  /content/menu/images/" + ctr_excel_list[overview_page][3][2].replace(".png", "").strip() + ".png 10"
	...
	for i in range(len(directory_lang)): 									#Uses directories from English CTR to look for all CTR's in correct order
		ctr_filename.append(glob.glob(__CTR_dir + "/CTR_" + airline_name + "_" + ctr_date + "_" + directory_lang[i] + ".xls"))
		if (len(ctr_filename[i]) < 1):
			print "CTR_" + airline_name + "_" + ctr_date + "_" + directory_lang[i].strip() + ".xls is missing"
			exit()
		elif (len(ctr_filename[i]) > 1):
			print "There are conflicting CTR_" + airline_name + "_" + ctr_date + "_" + directory_lang[i] + ".xls files"
			exit()
			...
			if (subcategory_type == "movies" or subcategory_type == "tv"):
				...
				for i in range(0,len(menu_metadata[l][c])):
					...
					if (airline_name == "ana" or airline_name == "thatana"):
						...
						if (new_holdover == "H"):
							video_date = "-" + '%s-%s-%s' % ((datecode[1] - 1), datecode[2], datecode[0])
						else: video_date = date
					else:
						date = ""
						video_date = ""
					title = menu_metadata[l][c][i][7].replace(" ", " ")
					if (isinstance( title, float )): title = str(int(title))
					else: title = title.rstrip('"').lstrip('"').strip().replace("#","@")
					episode = menu_metadata[l][c][i][8].replace(" ", " ")
					...
					if (subcategory != ""):
						if ("poster" not in cataList[l][c][0][4] and len(subcategory.split('(')[0].strip()) > 18):
					...
					if (subcategory_type == "music"):
						...
						if (content_provider.lower() != "digecor" or (studio.lower() == "sony" or studio.lower() == "paramount")):
							if (airline_name in __AirlineMap):
								airline_ID = __AirlineMap[airline_name] + "-"
							else:
								print "No airline ID found for " + airline_name
						...
						if (QA == "QA2"):
							if (source_filename == ""):
								video_filename_langs = GetFileNameLanguages(menu_metadata[menuLangENG][c][i])
								video_file_base = airline_ID + TitleToFilename(english_title.replace("'", "").lower().title() + english_episode.replace("'", "").lower().title())
								video_filename = FindBestFileNameMatch(video_file_base, video_filename_langs, temp_list)
								if (video_filename != "MISSINGVIDEO"):
									EditCTR((overview_page + c + 1), (i + 1), 37, video_filename)
							else:
				...
				for i in range(0,len(menu_metadata[l][c])):
					...
					if (cataList[l][c][0][5].strip().lower() == "podcasts"):
						...
						synop_basefile = cataList[l][c][0][6].strip()
						source_filename = menu_metadata[l][c][i][7].rstrip('"').lstrip('"').strip()
						if (source_filename == ""):
							print "Filename for " + english_title.encode('ascii', 'ignore') + " in " + cataList[menuLangENG][c][0][0].strip() + " for the " + directory_lang[l].strip().upper() + " menu is missing."
							exit()
						Games(directory_lang[l].strip())
						...
						ad_list = GetAdList("", menu_metadata[l][c][i][6].strip())
