...
def WriteSurveyCat(survey_title):
	...
	if (os.path.exists(__OutputDir + "/config/test.sml")):
		shutil.copy(__OutputDir + "/config/test.sml", __OutputDir + "/menu/survey/" + TitleToFilename(survey_title) + str(l + 1) + ".sml")
	else:
			...
			if (category_type == "movies" or category_type == "tv"):
				...
				for i in range(0,len(menu_metadata[l][see])):
					...
					if (AmIaFloat(menu_metadata[l][see][i][31]).lower() != "none" and AmIaFloat(menu_metadata[l][see][i][31]) != ""):
						ad2 = AmIaFloat(menu_metadata[l][see][i][31])
					else: ad2 = ""
					if (ad1 != "" or ad2 != ""):
					...
					if (len(synopsis) > 350):
						__Warnings.append("Synopsis for " + english_title + " " + english_episode + " in " + AmIaFloat(cataList[menuLangENG][c][0][0]).strip() + " in the " + directory_lang[l].strip().upper() + " menu is over 350 char and will look stupid!")
					if (AmIaFloat(menu_metadata[l][see][i][29]).rstrip('"').lstrip('"') != ""):
						...
						if (len(synopsis) > 410):
							__Warnings.append("Combined synopsis for " + english_title + " " + english_episode + " in " + AmIaFloat(cataList[menuLangENG][c][0][0]).strip() + " in the " + directory_lang[l].strip().upper() + " menu is over 350 char and will look stupid!")
					...
					if (subcategory != ""):
						if (len(subcategory.split('(')[0].strip()) > 18):
							__Warnings.append("Sub-category in " + AmIaFloat(cataList[menuLangENG][c][0][0]).strip() + " for the " + directory_lang[l].strip().upper() + " menu is over 18 char and will look stupid!")
					...
					if (subcategory_type == "music"):
						...
						GetSynop(directory_lang[l].strip(), category_type)
				if (len(subcategory_itemcount) < (len(cataList[l][c]) - all_videos_yes)):
				...
				WriteToMml(writeMe)
				f.write((indent * "\t") + "</menuitem>\n")
			elif (category_type == "ads" and os.path.exists(__OutputDir + "/config/" + TitleToFilename(AmIaFloat(cataList[menuLangENG][c][0][0].split('(')[0]).strip() + str(l + 1)) + ".txt")):
				...
				for i in range(0,len(menu_metadata[l][see])):
					...
					if (category_type == "podcasts"):
						...
						flag = "a"
						GetImage(title, song, image_directory)
						ad_list = GetAdList("", AmIaFloat(menu_metadata[l][see][i][6]))
