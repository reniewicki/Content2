...
def EditCTR(sheet, row, col, value):
	for i in range(len(ctr_edit_list)):
		ctr_edit_list[i].get_sheet(sheet).write(row, col, value)
	return
...
if __name__ == "__main__":
	...
	ctr_date = sys.argv[3]
	ctr_edit_list = []
	print "DO NOT open any excel files for " + airlineID + " while renaming files\nWORKING, Please be patient\n"
	...
	if (os.path.exists(ctr_eng_filename)):
		try:
			open(ctr_eng_filename, "r+")
		except IOError:
			print "Please close excel file and restart rename script:\n" + ctr_eng_filename
			exit()
		readXL = open_workbook(ctr_eng_filename)
		rename_excel_list = BuildListFromExcel(readXL)
	else:
		print "There is no ENG CTR for " + airlineID
		exit()
	for i in range(len(ctr_filename)):
		if (os.path.exists(ctr_filename[i])):
			try:
				open(ctr_filename[i], "r+")
			except IOError:
				print "Please close excel file and restart rename script:\n" + ctr_filename[i]
				exit()
			ctr_edit_list.append(copy(open_workbook(ctr_filename[i], formatting_info=True)))
		else:
			...
			if (cataList[j][5] == "movies" or cataList[j][5] == "tv"):
				for k in range(1, len(rename_excel_list[sheet])):
					...
					if (ext == ".mkv"):
						...
						if (episode != ""):
							title = TitleToFilename(mainTitle.replace("'", "").lower().title()) + "-" + TitleToFilename(episode.replace("'", "").lower().title())
						else:
							title = TitleToFilename(mainTitle.replace("'", "").lower().title())
						filebase = airlineID + "-" + title + date + languages
						currentFileName2 = AmIaFloat(rename_excel_list[sheet][k][(col+1)])
						if (currentFileName2 != ""): 
							__ConflictingFiles.append(currentFileName.encode('ascii', 'ignore') + ext + " AND " + currentFileName2.encode('ascii', 'ignore') + ext + "\nto " + airlineID + "-" + title + date + "\n")
							continue
					else:
						col_plus = 1
						col = 40
					...
					if(os.path.exists(__GenerationDir + currentFileName.strip() + ext) and languages != "fault"):
						...
						EditCTR(sheet, k, (col + col_plus), "/content/library/l7/airlinespecific/" + airlineID.lower() + "/" + filebase + ext + enc)
					else:
	...
	for i in range(len(ctr_edit_list)):
		...
		time.sleep(4)
		ctr_edit_list[i].save(ctr_filename[i])
	if (__ConflictingFiles != "" and ext == ".mkv"):
		print "Manually change files with appropriate languages!:"
		for conflicts in __ConflictingFiles:
			print conflicts
	if (__MissingFiles != ""):
		print "Missing Files:"
